 ...
 *
 * Additionally, three consecutive fail attempts on password entry will result
 * the phone to be locked out.
 *
 * Problem
...
class SmartPhone
{
    ...
    const STATE_LOCKED_OUT = 3;
    const MAX_NUMBER_OF_PASSWORD_ATTEMPTS = 3;
        ...
        if (self::STATE_OFF == $this->state) {
            ...
            if ($this->requirePassword) {
                if ($this->passwordCounter >= 3) {
                    $this->setState(self::STATE_LOCKED_OUT);
                } else {
            ...
            $this->setState(self::STATE_OFF);
        } else if (self::STATE_LOCKED_OUT == $this->state) {
            $this->setState(self::STATE_OFF);
        }
